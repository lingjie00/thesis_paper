...
best performing model [@gu_empirical_2019]. A recent paper
on asset pricing models uses Autoencoder, another class of
advanced neural network models, found that the inclusion of
economic theory in estimation dramatically improves the
model performance [@gu_autoencoder_2019]. The Generative
Adversarial Network (GAN) model proposed by Chen et al.
(2019) similarly attempts to improve the neural network
model performance by including the no-arbitrage
condition in the model estimation.

Different papers have estimated their models based on
various factors. For example, Fama and French (1993, 2015,
2017) expanded their factor models from three to five and six
factors. While many other papers have explored the
...
than the five factor model when estimated with a linear
model [@feng_taming_2020]. However,
Chen et al. (2019) found that all factors taken into
consideration have
contributed to the GAN model. Furthermore, including
macroeconomic data further improves the model
performance, aligned with another research paper's finding
...
sorted assets (Fama & French, 1993). Papers in asset-pricing
...
discriminative network that performs asset and factor
selection to maximise the pricing error.
...
tasks [@houdt_review_2020]. This paper uses LSTM to extract
the dynamic information from macroeconomic data.

The main contribution of this paper is in testing the
external validity of Chen et al. (2019)'s model with the
United Kingdom (U.K.) London Stock Exchange (LSE) data. Most empirical
papers are tested on the data-rich United States data, with
around 20% of the papers in top finance journals
investigating countries outside the United States (U.S.)
[@karolyi_home_2012]. Chen et al. (2019) model was also
trained based on the U.S. data. Therefore, the model performance
in a different context, such
as the U.K., remains to be answered and explored in this
paper.
There are several researches done on LSE data
but none answers the question this paper seeks to answer.
The existing papers are either exploring a limited
...
results. Besides the dropout strategy [@srivastava_dropout_2014]
adopted by Chen et al. (2019), this paper further
includes early stopping rules [@yao_early_2007], allowing
...
for the explanation. [@geron_hands-machine_2017]
