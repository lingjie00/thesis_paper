...
constant by using portfolio provided by Fama French.
...
<!--break-->
Popular ways to estimate the structure forms of SDF revolve
around linear models (using linear regression), non-linear
models (using ensemble and boosting tree models, feed
forward neural network). Linear model generally works well.
Although non-linear methods improved the performance, the
...
outperform both kind of models.
<!-- Explain why the validation strategy is insufficient -->
We attempt to validate the Chen's claim by
