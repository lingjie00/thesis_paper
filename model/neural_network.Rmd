...
$\mathbf{X}$ and output $y=f(\mathbf{X})$ and interaction
effects between input data $\mathbf{X}$. In this paper we
...
\@ref(fig:feedforward)), each of the input data $\mathbf{X} =
...
layer, a back-propagation training algorithm is then used to
train the network. Back-propagation minimises a given loss
...
$$
\text{Loss: } E(\mathbf{X}, \mathbf{\omega}) = E\left[ \mathbf{y} - \hat{f}(\mathbf{X})\right]^2
$$

With gradient:

$$
\frac{\partial E(\mathbf{X}, \mathbf{\omega})}{\partial \omega_{ij}^k}
= \frac{\partial E(\mathbf{X}, \mathbf{\omega})}{\partial a_j^k}
...
E(\mathbf{X}, \mathbf{\omega})}{\partial \mathbf{\omega}}
$$
