...
exist. Let $R_{t+1, i}$ denote the asset $i$'s return in
...
condition, we have the following equation.
$$
...
Let $\bold{\omega}_t$ denote the SDF weights used to
construct the tangency portfolio based on all the assets in
the market, and
$F_{t+1} := \bold{\omega}_t^T R^e_{t+1}$ denote the tangency
portfolio.
...
Since tangency portfolio occurs at the mean-variance
frontier where $\rho(M_{t+1}, F_{t+1})=-1$,
tangency portfolio is perfectly correlated to SDF
and is an affine transformation of the SDF
...
$$
M_{t+1} = 1 - F_{t+1} = 1 - \bold{\omega}_t^T R^e_{t+1}
$$
...
$$
E\left[\left(1-\bold{\omega}_t^T R^e_{t+1}\right)R^e_{t+1, i}\right] = 0
$$


## Pricing loss function {#loss_function}

The no-arbitrage condition reduced the asset-pricing problem
into estimating SDF weight function $\omega$.
...
pricing. In that case, we can define a function $g(I_t, I_{t, i})$
...
I_{t, i})$ and $g(I_t, I_{t, i})$ can be described as the
...
asset excess returns, which will be the key to building our
GAN model.
